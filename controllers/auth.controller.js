...
// Login user and return JWT in response body (no cookies)
exports.login = async (req, res, next) => {
  try {
    ...
    // Return the token in response body (not cookies)
    res.status(200).json({
      status: 200,
      message: "Login Success",
      token: `Bearer ${token}`, // Use Bearer token format
...
exports.sendEmail = async (req, res, next) => {
  ...
  const expiryTime = 300; // Token expiration time (5 minutes)
